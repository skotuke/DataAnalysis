...
for j=1:sweeps
    ...
    AP_actual_sizes=AP_sizes_without_first-AP_mins; %the list of actualAP sizes. baseline min precedes it action potential
    AP_actual_sizes_average=(sum(AP_actual_sizes))/(AP_times_number-1); %AP average size for a sweep
    ...
    AP_actual_sizes_table(1:(AP_times_number-1),j)=AP_actual_sizes;
...
frequency_list=frequency_list(1:(data,3));
...
excel_name = sprintf('%s\\..\\Output\\%s\\current_steps.xlsx', path, output_folder); %it tells the full path of the file
xlswrite(excel_name, {filename}, m, A1);
xlswrite(excel_name, {'Frequency'}, m, C1);
xlswrite(excel_name, frequency_list, m, C2);
xlswrite(excel_name, {'Step'}, m, B1);
xlswrite(excel_name, step_number, m, B2);
xlswrite(excel_name, {'AP actual size'}, m, D1);
xlswrite(excel_name, AP_actual_sizes_averages_list, m, D2;
xlswrite(excel_name, {'AP normalised'}, m, E1);
xlswrite(excel_name, AP_sizes./AP_sizes(1) , m, E2;
xlswrite(excel_name, {'Average ISI'}, m, F1);
xlswrite(excel_name, ISI_average_list, m, F2);

excel_name = sprintf('%s\\..\\Output\\%s\\isi_values.xlsx', path, output_folder);
xlswrite(excel_name, {filename}, m, A1);
...
excel_name = sprintf('%s\\..\\Output\\%s\\AP_nomalised.xlsx', path, output_folder);
xlswrite(excel_name, {filename}, m, A1);
...
excel_name = 'AP_actual_sizes.xlsx';
xlswrite(excel_name, {filename}, m, A1);
xlswrite(excel_name, {'Actual AP'},m, A3);
xlswrite(excel_name, transpose(step_number),m, B3);
xlswrite(excel_name, {'Step'}, m, B2);
xlswrite(excel_name, AP_actual_sizes_table, m+1);