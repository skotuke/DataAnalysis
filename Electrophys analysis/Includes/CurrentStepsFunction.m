...
excel_name = sprintf('%s\\current_steps_%s.xlsx', location, name); %it tells the full path of the file
...
excel_name = sprintf('%s\\ISI_values_%s.xlsx', location, name);
xlswrite(excel_name, {filename}, m, 'A1');
...
excel_name = sprintf('%s\\AP_normalised_%s.xlsx', location, name);
xlswrite(excel_name, {filename}, m, 'A1');
...
excel_name = sprintf('%s\\AP_actual_sizes_%s.xlsx', location, name);
xlswrite(excel_name, {filename}, m, 'A1');
